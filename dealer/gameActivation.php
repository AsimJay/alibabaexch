        ...
        <div class="table-responsive">
            <table id="transactionTable" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    $query = "SELECT * FROM users WHERE `users`.`did` = $dealerID";
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        foreach ($games as $game) {
                            echo "<td>";
                            $buttonID = $game . "-" . $row['id']; // Create a unique button ID using game and userID
                            if ($row[$game] == 0) {
                                echo '<a href="allowGame.php?user=' . $row['id'] . '&game=' . $game . '" class="btn btn-primary">Activate</a>';
                            } else {
                                echo '<a href="disableGame.php?user=' . $row['id'] . '&game=' . $game . '" class="btn btn-danger">Deactivate</a>';
                            }
                            echo "</td>";
                        }
                        echo "</tr>";
