...
if (isset($_GET['userId'])) {
    ...
    if ($result) {
        ...
        VALUES (NULL, '$userId', 'Spent', $amount, CURRENT_TIMESTAMP, '$combinedString');";

        $result = mysqli_query($con, $sql);

        //Getting the number;
        // checking if the number already exsists
        $sql = "SELECT * FROM biddetails WHERE bid_number = $number AND user_id = $userId";
        $result = mysqli_query($con, $sql);

        if (mysqli_num_rows($result) > 0) {
            $sql = "UPDATE biddetails SET bid_amount = bid_amount + $amount WHERE user_id = $userId AND bid_number = $number";
            $result = mysqli_query($con, $sql);
        } else {
        ...
        $sql3 = "UPDATE `number_table` SET `amount` = `amount` + $amount WHERE `id` = $number+1;";
