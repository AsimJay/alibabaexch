...
if(!$_GET['userID']){
    $url = "https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=4";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=6");
    // exit;
}
$userId = $_GET['userID'];
$game = $_GET['game'];
...
function purchaseDigits($userId, $digit, $price, $gameCon, $con, $digitsArray){
    $totalBidSql = "SELECT bid_amount FROM secondbiddetails WHERE bid_number = $digit AND user_id = $userId";
    ...
    if(checkBid($totalBid, $price)){
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=6";
        echo "<script>window.location.href = '$url';</script>";
    }else{
...
function checkBalance($userId, $price, $digitsArray, $con){
    ...
    if($currentBalance < $totalAmount ){
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=3";
        echo "<script>window.location.href = '$url';</script>";
        // header("Location: https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=3");
        // exit;
    }else{
...
function recordTransaction($userId, $digit, $price, $gameCon){
    ...
    if($result){
        //Insert into secondbiddetails
        $sql = "SELECT * FROM secondbiddetails WHERE bid_number = " . $digit . " AND user_id = " . $userId;
        $result = mysqli_query($gameCon, $sql);
        if (mysqli_num_rows($result) > 0) {
            $sql = "UPDATE secondbiddetails SET bid_amount = bid_amount + " . $price . ", bid_date = CURRENT_TIMESTAMP WHERE user_id = " . $userId . " AND bid_number = " . $digit;
            $result = mysqli_query($gameCon, $sql);
        } else {
            $sql = "INSERT INTO secondbiddetails(user_id, bid_number, bid_amount, bid_date)
...
function updateNumberTable($digit, $userId, $price, $gameCon){
    $sql3 = "UPDATE second_digit SET amount = amount ". " + " . $price . " WHERE id = " . ($digit + 1);
...
function logError(){
    $url = "https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=4";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=4");
    // exit;
}
...
if (!$marketSuspend) {
    ...
    if (isset($_GET['userID'])) {
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=1&b=" . $balance . "&gameData=" . $game;
        echo "<script>window.location.href = '$url';</script>";
        // header("Location: https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=1&b=" . urlencode(getUpdatedBalance($userId, $price, $digitsArray, $con)) . "&gameDB=" . $gameDB);
        // exit;
        // purchaseDigits($userId, $digitsArray[0], $price, $gameCon, $con);
            // echo "Error inserting data: " . mysqli_error($con);
    }
}else{
    $url = "https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=2";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/multiClose.php?m=2");
    // exit;
}
//else {
//     $response = array(
