...
if(!$_GET['userID']){
    $url = "https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=4";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=6");
    // exit;
}
$userId = $_GET['userID'];
$game = $_GET['game'];
...
function purchaseDigits($userId, $digit, $price, $gameCon, $con, $digitsArray){
    ...
    if(checkBid($totalBid, $price)){
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=6";
        echo "<script>window.location.href = '$url';</script>";
    }else{
...
function checkBalance($userId, $price, $digitsArray, $con){
    ...
    if($currentBalance < $totalAmount ){
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=3";
        echo "<script>window.location.href = '$url';</script>";
        // header("Location: https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=3");
        exit;
    }else{
...
function logError(){
    $url = "https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=4";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=4");
    // exit;
}
...
if (!$marketSuspend) {
    ...
    if (isset($_GET['userID'])) {
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=1&b=" . $balance . "&gameData=" . $game;
        echo "<script>window.location.href = '$url';</script>";
        // header("Location: https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=1&b=" . urlencode(getUpdatedBalance($userId, $price, $digitsArray, $con)) . "&gameData=" . $gameDB);
        // exit;
        // purchaseDigits($userId, $digitsArray[0], $price, $gameCon, $con);
            // echo "Error inserting data: " . mysqli_error($con);
    }
}else{
    $url = "https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=2";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/kainchi.php?m=2");
    // exit;
}
//else {
//     $response = array(
