...
if (!$marketSuspend) {
    if (isset($_GET['userId'])) {
        ...
        VALUES (NULL, '$userId', 'Spent on second digit', $amount, CURRENT_TIMESTAMP, '$combinedString');";

            $result = mysqli_query($con, $sql);

            //Getting the number;
            // checking if the number already exsists
            $sql = "SELECT * FROM secondbiddetails WHERE bid_number = $number AND user_id = $userId";
            $result = mysqli_query($con, $sql);

            if (mysqli_num_rows($result) > 0) {
                $sql = "UPDATE secondbiddetails SET bid_amount = bid_amount + $amount, bid_date = CURRENT_TIMESTAMP WHERE user_id = $userId AND bid_number = $number";
                $result = mysqli_query($con, $sql);
            } else {
            ...
            $sql3 = "UPDATE `second_digit` SET `amount` = `amount` + $amount WHERE `id` = $number+1;";
