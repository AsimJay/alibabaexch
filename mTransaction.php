...
if(!$_GET['userID']){
    $url = "https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=4";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=6");
    // exit;
}
$userId = $_GET['userID'];
$game = $_GET['game'];
$gameDB = str_replace("u423067381_", "", $game);
$digits = $_GET['number'];
$price = $_GET['price'];
// echo $gameDB;
//Decoding the digits
...
if ($gameCon->connect_error) {
    die("Connection failed: " . $gameCon->connect_error);
}

function purchaseDigits($userId, $digit, $price, $gameCon, $con, $digitsArray){
    $totalBidSql = "SELECT bid_amount FROM biddetails WHERE bid_number = $digit AND user_id = $userId";
    ...
    if(checkBid($totalBid, $price)){
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=6";
        echo "<script>window.location.href = '$url';</script>";
    }else{
            ...
            if(recordTransaction($userId, $digit, $price, $gameCon)){
                   ...
                   if(updateNumberTable($digit, $userId, $price, $gameCon)){
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            // echo "<script>alert('Done Purchasing')</script>";
                            // header("Location: https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=1");
                            // exit;
                        }
                    }else{
                        logError();
...
function checkBalance($userId, $price, $digitsArray, $con){
    ...
    if($currentBalance < $totalAmount ){
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=3";
        echo "<script>window.location.href = '$url';</script>";
        // header("Location: https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=3");
        // exit;
    }else{
...
function checkBid($totalBid, $price){
    ...
    if($totalBid > 2000 || ($totalBid + $price) > 2000){
        // echo "bid limit exceed";
        $bidLimitExceed = true;
    }
...
function recordTransaction($userId, $digit, $price, $gameCon){
    ...
    if($result){
        ...
        $sql = "SELECT * FROM biddetails WHERE bid_number = " . $digit . " AND user_id = " . $userId;
        $result = mysqli_query($gameCon, $sql);
        if (mysqli_num_rows($result) > 0) {
            $sql = "UPDATE biddetails SET bid_amount = bid_amount + " . $price . ", bid_date = CURRENT_TIMESTAMP WHERE user_id = " . $userId . " AND bid_number = " . $digit;
            $result = mysqli_query($gameCon, $sql);
        } else {
            $sql = "INSERT INTO biddetails(user_id, bid_number, bid_amount, bid_date)
...
function updateNumberTable($digit, $userId, $price, $gameCon){
    $sql3 = "UPDATE number_table SET amount = amount ". " + " . $price . " WHERE id = " . ($digit + 1);
...
function logError(){
    ...
    echo '<script>alert(hello);</script>';
        // echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=4");
    // exit;
}
...
if (!$marketSuspend) {
    ...
    if (isset($_GET['userID'])) {
        ...
        $url = "https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=1&b=" . $balance . "&gameData=" . $game;
        echo "<script>window.location.href = '$url';</script>";
        // header("Location: https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=1&b=" . urlencode(getUpdatedBalance($userId, $price, $digitsArray, $con)) . "&gameDB=" . $gameDB);
        // exit;
        // purchaseDigits($userId, $digitsArray[0], $price, $gameCon, $con);
            // echo "Error inserting data: " . mysqli_error($con);
    }
}else{
    $url = "https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=2";
        echo "<script>window.location.href = '$url';</script>";
    // header("Location: https://{$_SERVER['HTTP_HOST']}/multiDigit.php?m=2");
    // exit;
}
//else {
//     $response = array(
